...
namespace c
{
	...
	inline ImVec4 accent = ImColor(255, 255, 255);
...
inline void ToolTip(const char* Label, const char* Desc, const char* Icon, bool State) {
	...
	ImVec2 TextSize = FrameWork::Misc::CalcTextSize(FrameWork::Assets::InterBold, FrameWork::Assets::InterBold->FontSize, Desc);
	ImVec2 IconTextSize = FrameWork::Misc::CalcTextSize(FrameWork::Assets::FontAwesomeSolid, FrameWork::Assets::FontAwesomeSolid->FontSize, Icon);
	...
	ToolTipAnim->second.TextColor = ImLerp(ToolTipAnim->second.TextColor, Hovered ? ImColor(128, 128, 128) : ImColor(128, 128, 128, 0), ImGui::GetIO().DeltaTime * 12);
	ToolTipAnim->second.IconColor = ImLerp(ToolTipAnim->second.IconColor, Hovered ? ImVec4(ImColor(160, 80, 220)) : ImVec4(138.f/255.f, 43.f/255.f, 226.f/255.f, 0.f), ImGui::GetIO().DeltaTime * 8);
	...
	DrawList->AddText(FrameWork::Assets::FontAwesomeSolid, FrameWork::Assets::FontAwesomeSolid->FontSize, ImVec2(Pos.x + Padding / 2, Pos.y + ((TextSize.y + Padding) / 2 - IconTextSize.y / 2)), ImGui::GetColorU32(ToolTipAnim->second.IconColor), Icon);
	...
	if (ToolTipAnim->second.SlideX < TextSize.x + Padding) {
		...
		DrawList->AddText(FrameWork::Assets::InterBold, FrameWork::Assets::InterBold->FontSize, ImVec2(Pos.x + Padding / 2 + (IconTextSize.x + Spacing), Pos.y + Padding / 2), ImGui::GetColorU32(ToolTipAnim->second.TextColor), ClippedDesc.c_str());
	}
	else {
		DrawList->AddText(FrameWork::Assets::InterBold, FrameWork::Assets::InterBold->FontSize, ImVec2(Pos.x + Padding / 2 + (IconTextSize.x + Spacing), Pos.y + Padding / 2), ImGui::GetColorU32(ToolTipAnim->second.TextColor), Desc);
	}
...
bool ImGui::ButtonEx(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags, bool bToolTip, const char* ToolTipMsg, const char* ToolTipIcon)
{
	...
	const ImU32 inside_hover_col = GetColorU32(ImVec4(ImColor(138, 43, 226, (int)it_anim->second.closing_alpha)));
	...
	PushStyleColor(ImGuiCol_Text, ColorConvertFloat4ToU32(ImColor(128, 128, 128, (int)it_anim->second.label_alpha)));
	RenderTextClipped(bb.Min + style.FramePadding, bb.Max - style.FramePadding, label, NULL, &label_size, style.ButtonTextAlign, &bb);
	PopStyleColor();
...
bool ImGui::SubTab(const char* Label, bool Enabled)
{
	...
	// Padding fixo
	const ImVec2 padding = ImVec2(14, 7);
	ImRect rect(pos, pos + ImVec2(text_size.x + padding.x * 2, text_size.y + padding.y * 2));
	...
	// Cores
	ImColor activeBg = ImColor(138, 43, 226, 180);
	ImColor activeText = ImColor(255, 255, 255, 255);
	ImColor inactiveText = ImColor(160, 160, 160, 255);

	// Renderiza o fundo apenas se a tab estiver ativa ou sob o mouse
	if (Enabled || hovered) {
		DrawList->AddRectFilled(rect.Min, rect.Max, activeBg, 6.f);
	}
	...
	if (Enabled) {
		...
		text_color = inactiveText;
	}

	// Texto centralizado
	ImVec2 textPos(
		rect.Min.x + (rect.GetWidth() - text_size.x) * 0.5f,
		rect.Min.y + (rect.GetHeight() - text_size.y) * 0.5f
	);

	DrawList->AddText(FrameWork::Assets::InterBold, 0.0f, textPos, text_color, Label);
...
bool ImGui::Tab(const char* Label, const char* Icon, bool Enabled)
{
	...
	ImU32 bg_color = Enabled ? ImColor(138, 43, 226, 255) : ImColor(16, 16, 16, 255);
	ImU32 icon_color = Enabled ? ImColor(255, 255, 255, 255) : ImColor(160, 80, 220, 180);
	ImU32 text_color = ImColor(255, 255, 255, Enabled ? 255 : 180);
	window->DrawList->AddRectFilled(total_bb.Min, total_bb.Max, bg_color, 6.0f);
	PushFont(FrameWork::Assets::FontAwesomeSolid14);
	ImVec2 icon_pos = total_bb.Min + ImVec2(12.0f, tab_height / 2.0f - icon_size.y / 2.0f);
	window->DrawList->AddText(icon_pos, icon_color, Icon);
	PopFont();

	if (Enabled) {
		ImVec2 separator_pos = total_bb.Min + ImVec2(34.0f, tab_height / 2.0f - label_size.y / 2.0f);
		window->DrawList->AddText(total_bb.Min + ImVec2(30.0f, tab_height / 2.0f - label_size.y / 2.0f), ImColor(255, 255, 255, 100), "|");
	}

	PushFont(FrameWork::Assets::InterMedium);
	ImVec2 text_pos = total_bb.Min + ImVec2(40.0f, tab_height / 2.0f - label_size.y / 2.0f);
...
bool ImGui::TabHeaderButton(const char* ButtonName, bool enabled, int direction, int TabIndex)
{
	...
	ImColor TextColor = enabled ? ImColor(255, 105, 180, (int)(GetStyle().Alpha * 255)) : ImColor(1.f, 1.f, 1.f, 0.4f * GetStyle().Alpha);

	static ImDrawFlags Corner = ImDrawFlags_RoundCornersAll;
...
bool ImGui::TabHeader(const char* tabname, int* currentTab, std::vector<const char*> Tabs, int TabIndex)
{
	...
	SetCursorPos(ImVec2(-100, 10) + ImVec2(bb.GetWidth() / 2 - WidhtMax / 2, 12.5f));
...
bool ImGui::ScrollbarEx(const ImRect& bb_frame, ImGuiID id, ImGuiAxis axis, ImS64* p_scroll_v, ImS64 size_avail_v, ImS64 size_contents_v, ImDrawFlags flags)
{
	...
	if (axis == ImGuiAxis_Y)
		window->DrawList->AddRectFilledMultiColor(grab_rect.Min, grab_rect.Max, ImColor(75, 70, 175, 0), ImColor(75, 70, 175, 0), ImColor(255, 0, 0, (int)(GetStyle().Alpha * 155)), ImColor(255, 0, 0, (int)(GetStyle().Alpha * 155)), 5);
...
bool ImGui::Checkbox(const char* label, bool* v)
{
	...
	const float square_sz = 23;
	const ImVec2 pos = window->DC.CursorPos;
	const ImRect total_bb(pos, pos + ImVec2(square_sz + 10 + label_size.x, 20));
	...
	const ImRect check_bb(total_bb.Min, total_bb.Min + ImVec2(square_sz, square_sz));
	...
	if (std::abs(Value->second.a) < 0.9f)
		window->DrawList->AddRectFilled(check_bb.Min + ImVec2(check_bb.GetWidth() / 2 * Value->second.a + 1, check_bb.GetHeight() / 2 * Value->second.a + 1),
			check_bb.Max - ImVec2(check_bb.GetWidth() / 2 * Value->second.a + 1, check_bb.GetHeight() / 2 * Value->second.a + 1),
			ImColor(138, 43, 226, (int)(GetStyle().Alpha * 255)), 5);
	...
	if (label_size.x > 0.0f)
		RenderText(ImVec2(check_bb.Max.x + 5, total_bb.Min.y + total_bb.GetHeight() / 2 - label_size.y / 2), label);
...
bool ImGui::KeyBind(const char* label, int* Key, int* KeyState)
{
	...
	float target_width = (*Key >= 0 && *Key < IM_ARRAYSIZE(KeyNames)) ? (CalcTextSize(KeyNames[*Key]).x + 30) : 50.f;
	it_anim->second.a = ImLerp(it_anim->second.a, target_width, g.IO.DeltaTime * 10.f);

	float width = 130.0f;
	ImVec2 pos = window->DC.CursorPos;

	ImRect total_bb(pos, pos + ImVec2(width, 25));
	ImRect frame_bb(pos + ImVec2(width - 25 - it_anim->second.a, 0), pos + ImVec2(width - 20, 25));
	...
	if (*Key != 0 && g.ActiveId != id)
	{
		if (*Key >= 0 && *Key < IM_ARRAYSIZE(KeyNames))
			...
			strcpy(buf_display, "Unknown");
	}
	else if (g.ActiveId == id)
		strcpy(buf_display, "Press");

	bool is_none_or_press = (strcmp(buf_display, "None") == 0 || strcmp(buf_display, "Press") == 0);
	ImVec4 target_color = is_none_or_press ? ImVec4(1.f, 1.f, 1.f, 0.6f * GetStyle().Alpha) : ImVec4(1.f, 1.f, 1.f, 0.8f * GetStyle().Alpha);
	it_anim->second.b = ImLerp(it_anim->second.b, target_color, g.IO.DeltaTime * 10.f);

	window->DrawList->AddText(ImVec2(total_bb.Min.x, frame_bb.Min.y + frame_bb.GetHeight() / 2 - CalcTextSize(label).y / 2),
		ImColor(1.f, 1.f, 1.f, 0.8f * GetStyle().Alpha), label);

	window->DrawList->AddRectFilled(frame_bb.Min, frame_bb.Max, ImColor(14, 14, 14), 5);

	ImVec2 text_pos = ImVec2(frame_bb.Min.x + 7, frame_bb.Min.y + frame_bb.GetHeight() / 2 - CalcTextSize(buf_display).y / 2);
	window->DrawList->AddText(text_pos, GetColorU32(it_anim->second.b), buf_display);
...
bool ImGui::BeginCombo(const char* label, const char* preview_value, ImGuiComboFlags flags, int itemcount)
{
	...
	if (!(flags & ImGuiComboFlags_NoPreview))
	{
		...
		window->DrawList->AddCircleFilled(ImVec2(bb.Max.x - 14, bb.Min.y + total_bb.GetHeight() / 2), Value->second.a, ImColor(138, 43, 226, 255));
	}
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
	...
	const ImGuiStyle& style = g.Style;
	const ImGuiID id = window->GetID(label);
	const float full_width = GetWindowSize().x - 20;

	const ImVec2 label_size = CalcTextSize(label, NULL, true);
	const ImVec2 pos = window->DC.CursorPos;

	const float slider_height = 14.0f;
	const ImRect frame_bb(pos + ImVec2(0, label_size.y + 4), pos + ImVec2(full_width, label_size.y + 4 + slider_height));
	...
	if (!temp_input_is_active)
	{
		const bool input_requested_by_tabbing = temp_input_allowed && (g.LastItemData.StatusFlags & ImGuiItemStatusFlags_FocusedByTabbing);
		const bool clicked = hovered && IsMouseClicked(0, id);
		const bool make_active = (input_requested_by_tabbing || clicked || g.NavActivateId == id);
		if (make_active && clicked)
			SetKeyOwner(ImGuiKey_MouseLeft, id);
		if (make_active && temp_input_allowed && (input_requested_by_tabbing || (clicked && g.IO.KeyCtrl) || (g.NavActivateId == id && (g.NavActivateFlags & ImGuiActivateFlags_PreferInput))))
			temp_input_is_active = true;
	...
	if (value_changed)
	{
		...
		{
			int* val = (int*)p_data;
			int clamped_val = clamp(*val, *(const int*)p_min, *(const int*)p_max);
			*val = clamped_val;
		}
		MarkItemEdited(id);
	}

	static std::unordered_map<ImGuiID, SliderAnim> anims;
	auto& anim = anims[id];
	anim.anim = ImLerp(anim.anim, (float)percent / 100.0f * frame_bb.GetWidth(), 0.13f);
	anim.anim2 = ImLerp(anim.anim2, (float)percent, 0.13f);

	window->DrawList->AddRectFilled(
		frame_bb.Min, frame_bb.Max,
		ImColor(20, 20, 20, (int)(GetStyle().Alpha * 255)), 6.0f
	);

	window->DrawList->AddRectFilled(
		frame_bb.Min + ImVec2(1, 0),
		ImVec2(frame_bb.Min.x + anim.anim, frame_bb.Max.y),
		ImColor(138, 43, 226, (int)(GetStyle().Alpha * 180)), 6.0f
	);

	const float line_thickness = 4.0f;
	const float line_x = frame_bb.Min.x + anim.anim - 2.0f; // um pouco para dentro
	const float line_extend = 2.0f; // estende um pouco acima e abaixo do slider
	const ImVec2 line_start(line_x, frame_bb.Min.y - line_extend);
	const ImVec2 line_end(line_x, frame_bb.Max.y + line_extend);
	window->DrawList->AddLine(line_start, line_end, ImColor(255, 255, 255, 255), line_thickness);

	if (label_size.x > 0.0f)
	{
		window->DrawList->AddText(
			total_bb.Min,
			ImColor(255, 255, 255, int(0.9f * 255 * GetStyle().Alpha)),
			label
		);
	}
	...
	PushFont(FrameWork::Assets::InterBold);
	window->DrawList->AddText(
		ImVec2(frame_bb.Max.x - value_size.x, total_bb.Min.y),
		ImColor(138, 43, 226, 180),
		value_buf, value_buf_end
	);
	PopFont();
...
bool ImGui::ColorEdit4(const char* label, float col[4], ImGuiColorEditFlags flags)
{
	...
	if (label != label_display_end && !(flags & ImGuiColorEditFlags_NoLabel))
	{
		...
		it_anim->second.text = ImLerp(it_anim->second.text, label ? c::accent : c::text::text, g.IO.DeltaTime * 6.f);

		ImGui::PushFont(FrameWork::Assets::InterSemiBold);
		GetWindowDrawList()->AddText(window->DC.CursorPos + ImVec2(0, 4), GetColorU32(it_anim->second.text), label);

		GetWindowDrawList()->AddText(window->DC.CursorPos + ImVec2(GetContentRegionMax().x - (50 + CalcTextSize(buf).x), 4), GetColorU32(ImVec4(c::accent.x, c::accent.y, c::accent.z, 0.3f)), buf);
		ImGui::PopFont();
	}
...
bool ImGui::ColorButton(const char* desc_id, const ImVec4& col, ImGuiColorEditFlags flags, const ImVec2& size_arg)
{
	...
	const float default_size = GetFrameHeight();
	const ImVec2 pos = window->DC.CursorPos;
	const float width = GetContentRegionMax().x - ImGui::GetStyle().WindowPadding.x - 20;
...
bool ImGui::Selectable(const char* label, bool selected, ImGuiSelectableFlags flags, const ImVec2& size_arg)
{
	...
	PushFont(FrameWork::Assets::InterRegular);
	ImGui::PushStyleColor(ImGuiCol_Text, selected ? ImVec4(ImColor(138, 43, 226)) : ImVec4(ImColor(255, 255, 255)));
...
bool ImGui::BeginMenuEx(const char* label, const char* icon, bool enabled)
{
	...
	if (window->DC.LayoutType == ImGuiLayoutType_Vertical) // (window->Flags & (ImGuiWindowFlags_Popup|ImGuiWindowFlags_ChildMenu))
	{
		...
		if (g.HoveredWindow == window && child_menu_window != NULL)
		{
			...
			//GetForegroundDrawList()->AddTriangleFilled(ta, tb, tc, moving_toward_child_menu ? IM_COL32(0,255, 105, 180) : IM_COL32(128,0,0,128)); // [DEBUG]
		}
...
void ImGui::TabBarQueueReorderFromMousePos(ImGuiTabBar* tab_bar, ImGuiTabItem* src_tab, ImVec2 mouse_pos)
{
	...
	for (int i = src_idx; i >= 0 && i < tab_bar->Tabs.Size; i += dir)
	{
		...
		//GetForegroundDrawList()->AddRect(ImVec2(x1, tab_bar->BarRect.Min.y), ImVec2(x2, tab_bar->BarRect.Max.y), IM_COL32(255, 255, 105, 180));
...
static ImGuiTabItem* ImGui::TabBarScrollingButtons(ImGuiTabBar* tab_bar)
{
	...
	//window->DrawList->AddRect(ImVec2(tab_bar->BarRect.Max.x - scrolling_buttons_width, tab_bar->BarRect.Min.y), ImVec2(tab_bar->BarRect.Max.x, tab_bar->BarRect.Max.y), IM_COL32(255, 105, 180,255));
...
void ImGui::TabItemLabelAndCloseButton(ImDrawList* draw_list, const ImRect& bb, ImGuiTabItemFlags flags, ImVec2 frame_padding, const char* label, ImGuiID tab_id, ImGuiID close_button_id, bool is_contents_visible, bool* out_just_closed, bool* out_text_clipped)
{
	...
	if (out_text_clipped)
	{
		*out_text_clipped = (text_ellipsis_clip_bb.Min.x + label_size.x) > text_pixel_clip_bb.Max.x;
		//draw_list->AddCircle(text_ellipsis_clip_bb.Min, 3.0f, *out_text_clipped ? IM_COL32(255, 255, 105, 180) : IM_COL32(0, 255, 0, 255));
	}
